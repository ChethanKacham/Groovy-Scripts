...
def actionOne(toolDetails, projName, action, requestsCpu, requestsMemory, persistentStorage, backupStorage, logfileStorage, servicesNodeports) { // perform action 1    
    def jsonSlurper = new JsonSlurper()
    ...
    //println data
    //println token
    def curl_cmd = "curl -s -w '%{http_code}' -o response.txt -k -X PUT '$projectUrl' -H \"Authorization: Bearer $token\" -H 'Accept: application/json' -H 'Content-Type: application/json' -d '$data' > response_code.txt"
    //println (curl_cmd)
...
def main() {
   ...
   // String toolName=inputJson.environment.tool.name; 
    ...
    String persistentStorage = "9" 
    String backupStorage = "10"
    String logfileStorage = "11"
    String servicesNodeports = "12" 
    ...
    def status = ""
    def action = "Update Project"
    def createdStatus = actionOne(toolDetails, projName, action, requestsCpu, requestsMemory, persistentStorage, backupStorage, logfileStorage, servicesNodeports);
